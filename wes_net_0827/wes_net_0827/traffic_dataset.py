...
def get_adjacent_matrix(distance_file: str, num_nodes: int, id_file: str = None, graph_type="distance") -> np.array:
    ...
    A = np.zeros([int(num_nodes), int(num_nodes)])  # Constructing an all-0 adjacency matrix
...
if __name__ == '__main__':
    train_data = LoadData(data_path=["data/test17.csv", "data/test17.npz"], num_nodes=34, divide_days=[583, 146],
                          time_interval=1, history_length=6,
                          train_mode="train")

    print(len(train_data))
    print(train_data[0]["flow_x"].size())
    print(train_data[0]["flow_y"][:, :, 0][:, :, np.newaxis].size())
